...
control methodology:
    training one patient:
        it finds the best pre-ictal period and
...
def calculatePreIctalAndFeatureNumber(patient_id):
    t = time.process_time()
    contador=0;
    ...
    preictal_gridsearch_values=[30,35,40,45,50,55,60]
    k_features=[3,5,7,10,15,20]
    performance_values=np.zeros([len(preictal_gridsearch_values), len(k_features)])
    
    # iterating each pre-ictal period value
    for i in range(0,len(preictal_gridsearch_values)):  
        ...
        for kk in range(0,len(k_features)):
            ...
            for k in range(0,3):
                ...
                if k==0:
                    ...
                    testing_labels=getLabelsForSeizure(testing_features,preictal_gridsearch_values[i])
                    ...
                    training_labels_1=getLabelsForSeizure(training_features_1,preictal_gridsearch_values[i])
                    
                    # removing the class label for the feature vector, for one seizure
                    training_features_2=seizure_3_data[0:-1,:]
                    # retrieving the testing labels for one seizure
                    training_labels_2=getLabelsForSeizure(training_features_2,preictal_gridsearch_values[i])
                ...
                contador=contador+1
                print(str(contador) +" of "+ str(7*6*3) + " iterations")
    ...
    chosen_pre_ictal=preictal_gridsearch_values[best_set[0]]
    chosen_k_features=k_features[best_set[1]]
    
    print("Patient "+ str(patient_id) + ", Pre-Ictal: "+str(chosen_pre_ictal)+" min with "+str(chosen_k_features)+" features.")
    
    return [chosen_pre_ictal, chosen_k_features]

calculatePreIctalAndFeatureNumber(patient_id)
